...
public class ItemRegistries {
    ...
    // Spirit
    public static final DeferredItem<SpiritBottleItem> GIN = registerSpirit("gin", BlockRegistries.GIN, new Item.Properties().component(DataComponentTypeRegistries.SPIRIT_CONTENT, new SpiritContent(new FluidStack(FluidRegistries.GIN_SOURCE, FluidConstants.SPIRIT_BOTTLE_MAX_FLUID_CAPACITY))));
    public static final DeferredItem<SpiritBottleItem> WHISKY = registerSpirit("whisky", BlockRegistries.WHISKY, new Item.Properties().component(DataComponentTypeRegistries.SPIRIT_CONTENT, new SpiritContent(new FluidStack(FluidRegistries.WHISKY_SOURCE, FluidConstants.SPIRIT_BOTTLE_MAX_FLUID_CAPACITY))));
    public static final DeferredItem<SpiritBottleItem> VODKA = registerSpirit("vodka", BlockRegistries.VODKA, new Item.Properties().component(DataComponentTypeRegistries.SPIRIT_CONTENT, new SpiritContent(new FluidStack(FluidRegistries.VODKA_SOURCE, FluidConstants.SPIRIT_BOTTLE_MAX_FLUID_CAPACITY))));
    public static final DeferredItem<SpiritBottleItem> RUM = registerSpirit("rum", BlockRegistries.RUM, new Item.Properties().component(DataComponentTypeRegistries.SPIRIT_CONTENT, new SpiritContent(new FluidStack(FluidRegistries.RUM_SOURCE, FluidConstants.SPIRIT_BOTTLE_MAX_FLUID_CAPACITY))));
    public static final DeferredItem<SpiritBottleItem> TEQUILA = registerSpirit("tequila", BlockRegistries.TEQUILA, new Item.Properties().component(DataComponentTypeRegistries.SPIRIT_CONTENT, new SpiritContent(new FluidStack(FluidRegistries.TEQUILA_SOURCE, FluidConstants.SPIRIT_BOTTLE_MAX_FLUID_CAPACITY))));
    public static final DeferredItem<SpiritBottleItem> BRANDY = registerSpirit("brandy", BlockRegistries.BRANDY, new Item.Properties().component(DataComponentTypeRegistries.SPIRIT_CONTENT, new SpiritContent(new FluidStack(FluidRegistries.BRANDY_SOURCE, FluidConstants.SPIRIT_BOTTLE_MAX_FLUID_CAPACITY))));
    ...
    // TODO: 人工审查 - 2026-09-03 - 新增三个酒保控制物品:
    //  BARTENDER_STATUETTE (酒保雕像): 拥有者对酒保使用时切换无AI模式(不会进行任何AI计算);
    //  FREE_POUR_VOUCHER   (免单券):   拥有者对酒保使用时切换免单模式(AI取物不消耗容器物品);
    //  SEARCH_WAIVER       (寻物豁免券): 拥有者对酒保使用时切换寻物豁免模式
    //                                   (寻找物品超时时跳过寻找、假装已找到并继续后续流程)。
    //  原文件中没有这三个物品,本次为"酒保工作模式切换"功能添加。
    ...
    public static final DeferredItem<Item> SEARCH_WAIVER = register("search_waiver", Item::new);
    ...
    public static DeferredItem<FluidHolderItem> registerFluidHolder(String name, DeferredHolder<Fluid, FlowingFluid> fluid) {
        return registerFluidHolder(name, fluid, FluidConstants.SHAKER_MAX_FLUID_CAPACITY);
    }
    ...
    public static <T extends Block> DeferredItem<SpiritBottleItem> registerSpirit(String name, DeferredBlock<T> block, DeferredHolder<Fluid, FlowingFluid> fluid, Item.Properties properties) {
        return ITEM.register(name, (registryName) -> new SpiritBottleItem(block.get(), properties.setId(ResourceKey.create(Registries.ITEM, registryName))
                .component(DataComponentTypeRegistries.SPIRIT_CONTENT, new SpiritContent(new FluidStack(fluid, FluidConstants.SPIRIT_BOTTLE_MAX_FLUID_CAPACITY)))));
    }
